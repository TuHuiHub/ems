	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="emsClassRoomSelectorModalLabel">选择教室</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="emsClassRoomSelectorTable">
                    <thead>
                        <tr>
                            <th>id</th>
                                                        <th>roomName</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'emsClassRoomSelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "roomName" , "orderable" : false});
            	            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/ems/classRoom/selector/list" + (opts.params ? '?params=' + opts.params : ''), // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectEmsClassRoomOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeEmsClassRoomSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectEmsClassRoomOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeEmsClassRoomSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });

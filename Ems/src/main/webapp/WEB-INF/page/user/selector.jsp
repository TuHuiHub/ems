	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="bsysUserSelectorModalLabel">选择用户</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="bsysUserSelectorTable">
                    <thead>
                        <tr>
                            <th>ID</th>
                                                        <th>登录名</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'bsysUserSelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "loginName" , "orderable" : false});
            	            	cols.push ({ "data": "password" , "orderable" : false});
            	            	cols.push ({ "data": "createTime" , "orderable" : false});
            	            	cols.push ({ "data": "passwordTime" , "orderable" : false});
            	            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/bsys/user/selector/list", // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectBsysUserOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeBsysUserSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectBsysUserOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeBsysUserSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });

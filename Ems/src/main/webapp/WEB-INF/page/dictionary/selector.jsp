	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="bsysDictionarySelectorModalLabel">选择数据字典</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="bsysDictionarySelectorTable">
                    <thead>
                        <tr>
                            <th>ID</th>
                                                        <th>类型</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'bsysDictionarySelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "dicType" , "orderable" : false});
            	            	cols.push ({ "data": "val" , "orderable" : false});
            	            	cols.push ({ "data": "name" , "orderable" : false});
            	            	cols.push ({ "data": "description" , "orderable" : false});
            	            	cols.push ({ "data": "params" , "orderable" : false});
            	            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/bsys/dictionary/selector/list", // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectBsysDictionaryOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeBsysDictionarySelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectBsysDictionaryOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeBsysDictionarySelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });

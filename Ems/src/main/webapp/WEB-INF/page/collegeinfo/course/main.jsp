	...
	<script>
            $(document).ready(function() {
            	...
            	var cols = [
                	...
                	{ "data": "isOptional" , "orderable" : false},
                	// 下面在操作列中添加了修改和删除按钮
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置主键字段名称
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/collegeinfo/course/list?id="+${major.id}, // 请求列表数据的url
                        "type": "GET" // http方法
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='createCourse();'>创建</a> <a class='btn btn-primary btn-warning btn-sm' onclick='deleteCourses();'>批量删除</a> <a class='btn btn-success btn-success btn-sm' onclick='backToMajor();'>返回</a>");
            });

            function createCourse () {
            	location.href = "${pageContext.request.contextPath}/collegeinfo/course/create?id="+${major.id};
            }
            
            function backToMajor () {
            	location.href = "${pageContext.request.contextPath}/collegeinfo/major/back";
            }
            
            function updateCourse (id) {
            	location.href = "${pageContext.request.contextPath}/collegeinfo/course/update?id=" + id;
            }

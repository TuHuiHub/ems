    ...
    pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="majorSelectorModalLabel">选择专业</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="majorSelectorTable">
                    <thead>
                        <tr>
                            <th>ID</th>
                                                        <th>专业名</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'majorSelectorTable';
            	var cols = [];
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/collegeinfo/major/selector/list"
                        	+ (opts.params ? '?params=' + opts.params : ''), // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectmajorOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closemajorSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectmajorOK = function () {
             	...
             	// 确定按钮点击事件
                window.closemajorSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });

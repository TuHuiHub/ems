	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="emsTimeSlotSelectorModalLabel">选择时间段</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="emsTimeSlotSelectorTable">
                    <thead>
                        <tr>
                            <th>id</th>
                                                        <th>timeslot</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'emsTimeSlotSelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "timeslot" , "orderable" : false});
            	            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/ems/timeSlot/selector/list" + (opts.params ? '?params=' + opts.params : ''), // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectEmsTimeSlotOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeEmsTimeSlotSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectEmsTimeSlotOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeEmsTimeSlotSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });

	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="bsysMenuSelectorModalLabel">选择菜单</h4>
		    </div>
            <!-- /.row -->
            <div class="modal-body">
                <table width="100%" class="table table-striped table-bordered table-hover" id="bsysMenuSelectorTable">
                    <thead>
                        <tr>
                            <th>ID</th>
                                                        <th>名称</th>
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'bsysMenuSelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "name" , "orderable" : false});
            	            	cols.push ({ "data": "no" , "orderable" : true});
            	            	cols.push ({ "data": "icon" , "orderable" : false});
            	            	cols.push ({ "data": "url" , "orderable" : false});
            	            	cols.push ({ "data": "page" , "orderable" : false});
            	            	            	cols.push ({ "data": "parentName" , "orderable" : false});
            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/bsys/menu/selector/list", // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectBsysMenuOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeBsysMenuSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectBsysMenuOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeBsysMenuSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });
